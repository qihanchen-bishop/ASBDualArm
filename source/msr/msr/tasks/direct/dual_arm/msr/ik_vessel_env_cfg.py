...
import msr.tasks.direct.dual_arm.mdp as mdp
...
def _apply_low_memory_physx_buffers(env_cfg) -> None:
    """Use smaller PhysX GPU buffers for single-env play/data-collection runs."""
    ...
    env_cfg.sim.physx.gpu_heap_capacity = 2**26
    env_cfg.sim.physx.gpu_temp_buffer_capacity = 2**24
    env_cfg.sim.physx.gpu_max_soft_body_contacts = 2**22
    env_cfg.sim.physx.gpu_max_particle_contacts = 2**10
...
def _disable_robot_2_terms(env_cfg) -> None:
    """Keep IK vessel variants single-arm even when the base scene has dual arms."""
    env_cfg.actions.arm_2_action = None
    env_cfg.actions.gripper_2_action = None

    env_cfg.rewards.end_effector_2_position_tracking = None
    env_cfg.rewards.end_effector_2_orientation_tracking = None
    env_cfg.rewards.end_effector_2_position_tracking_fine_grained = None
    env_cfg.rewards.end_effector_2_orientation_tracking_fine_grained = None
    env_cfg.rewards.joint_2_vel = None

    env_cfg.commands.ee_2_pose = None

    env_cfg.observations.policy.pose_2_command = None
    env_cfg.observations.policy.pose_2_rel = None
    env_cfg.observations.policy.ee_2_pose = None
    env_cfg.observations.policy.jaw_pos_2 = None

    env_cfg.scene.ee_2_frame = None
    env_cfg.events.reset_robot_2_joints = None

    env_cfg.curriculum.end_effector_2_orientation_tracking_fine_grained = None
    env_cfg.curriculum.end_effector_2_position_tracking_fine_grained = None
    env_cfg.curriculum.joint_2_vel = None


def _enable_robot_2_ik_terms(env_cfg, use_rcm: bool) -> None:
    """Enable robot_2 IK control so both end-effectors are solved independently."""
    if use_rcm:
        env_cfg.actions.arm_2_action = DifferentialInverseKinematicsWithSoftRCMWorkspaceClampedActionCfg(
            asset_name="robot_2",
            joint_names=PSM_ARM_JOINT_NAMES,
        ...
        env_cfg.actions.arm_2_action = DifferentialInverseKinematicsWorkspaceClampedActionCfg(
            asset_name="robot_2",
            joint_names=PSM_ARM_JOINT_NAMES,
            body_name="psm_tool_tip_Link",
    ...
    # Keep policy observation style consistent with arm_1: expose ee pose, no command terms.
    env_cfg.commands.ee_2_pose = None
    env_cfg.observations.policy.pose_2_command = None
    env_cfg.observations.policy.pose_2_rel = None
    env_cfg.observations.policy.ee_2_pose = ObsTerm(
        func=mdp.ee_pose,
        params={"asset_cfg": SceneEntityCfg("robot_2", body_names="psm_tool_tip_Link")},
    )
    env_cfg.observations.policy.jaw_pos_2 = None

    env_cfg.events.reset_robot_2_joints = EventTerm(
...
@configclass
class MSRPSMVesselSemRewardEnvCfg(joint_pos_env_cfg.MSRPSMUpe6DualArmEnvCfg):
    """
    ...
    def __post_init__(self):
        # Post init of parent (MSRPSMUpe6DualArmEnvCfg)
        super().__post_init__()
        ...
        # Keep this variant single-arm.
        _disable_robot_2_terms(self)
        ...
        # ========================================================================
        # Parent (MSRPSMUpe6DualArmEnvCfg) disabled all reset events.
...
class MSRPSMVesselSemRewardEnvCfg_PLAY(MSRPSMVesselSemRewardEnvCfg):
    ...
    def __post_init__(self):
        ...
        # Play/data-collection runs only need a single env, so keep PhysX GPU
        # allocations aligned with the smaller dual-arm baseline config.
        _apply_low_memory_physx_buffers(self)
...
@configclass
class MSRPSMVesselSemRewardWithRCMEnvCfg(joint_pos_env_cfg.MSRPSMUpe6DualArmEnvCfg):
    """
    ...
    def __post_init__(self):
        ...
        # No gripper action (keep closed) and keep this variant single-arm.
        self.actions.gripper_1_action = None
        _disable_robot_2_terms(self)
...
@configclass
class MSRPSMDualArmVesselSemRewardEnvCfg(MSRPSMVesselSemRewardEnvCfg):
    """Dual-arm IK vessel env (non-RCM) with independent IK solve per arm."""

    def __post_init__(self):
        super().__post_init__()
        _enable_robot_2_ik_terms(self, use_rcm=False)


@configclass
class MSRPSMDualArmVesselSemRewardEnvCfg_PLAY(MSRPSMDualArmVesselSemRewardEnvCfg):
    """Play config for dual-arm IK vessel env (non-RCM)."""
...
@configclass
class MSRPSMDualArmVesselSemRewardWithRCMEnvCfg(MSRPSMVesselSemRewardWithRCMEnvCfg):
    """Dual-arm IK+RCM vessel env with independent IK solve per arm."""

    def __post_init__(self):
        super().__post_init__()
        _enable_robot_2_ik_terms(self, use_rcm=True)


@configclass
class MSRPSMDualArmVesselSemRewardWithRCMEnvCfg_PLAY(MSRPSMDualArmVesselSemRewardWithRCMEnvCfg):
    """Play config for dual-arm IK+RCM vessel env."""
...
@configclass
class MSRPSMDualArmVesselSemRewardConnectivityOnlyEnvCfg(MSRPSMDualArmVesselSemRewardEnvCfg):
    """Dual-arm IK env with only hard-connectivity reward enabled."""
...
@configclass
class MSRPSMDualArmVesselSemRewardConnectivityOnlyEnvCfg_PLAY(MSRPSMDualArmVesselSemRewardEnvCfg_PLAY):
    """Play dual-arm IK env with only hard-connectivity reward enabled."""
...
@configclass
class MSRPSMDualArmVesselSemRewardWithRCMConnectivityOnlyEnvCfg(MSRPSMDualArmVesselSemRewardWithRCMEnvCfg):
    """Dual-arm IK+RCM env with only hard-connectivity reward enabled."""
...
@configclass
class MSRPSMDualArmVesselSemRewardWithRCMConnectivityOnlyEnvCfg_PLAY(MSRPSMDualArmVesselSemRewardWithRCMEnvCfg_PLAY):
    """Play dual-arm IK+RCM env with only hard-connectivity reward enabled."""
